export const navItems = [
  { name: 'Home', link: '#' },
  { name: 'About', link: '#about' },
  { name: 'Projects', link: '#projects' },
  { name: 'Contact', link: '#contact' },
]

export const gridItems = [
  {
    id: 1,
    title: 'Collaboration First, Building Together with Clear Communication ',
    description: '',
    className: 'lg:col-span-3 md:col-span-6 md:row-span-4  md:h-[44vh]',
    imgClassName: 'w-full h-full',
    titleClassName: 'justify-end',
    img: '/b1.svg',
    spareImg: '',
  },
  {
    id: 2,
    title: 'Time Zones Are No Barrier — I Adapt to Your Schedule',
    description: '',
    className: 'lg:col-span-2 md:col-span-3 md:row-span-2',
    imgClassName: '',
    titleClassName: 'justify-start',
    img: '',
    spareImg: '',
  },
  {
    id: 3,
    title: 'My Tech Stack & Tools',
    description: '',
    className: 'lg:col-span-2 md:col-span-3 md:row-span-2 md:h-[20vh]',
    imgClassName: '',
    titleClassName: 'justify-center',
    img: '',
    spareImg: '',
  },
  {
    id: 4,
    title: 'Tech enthusiast with a passion for development.',
    description: '',
    className: 'lg:col-span-2 md:col-span-3 md:row-span-1 md:h-[15vh] text-sm',
    imgClassName: '',
    titleClassName: 'justify-start',
    img: '/grid.svg',
    spareImg: '/b4.svg',
  },

  {
    id: 5,
    title: ' Uncover insights, innovation, and impactful creations. ',
    description: 'More visionary and collaborative.',
    className: 'md:col-span-3 md:row-span-2 md:h-[35vh] lg:h-[38vh] ',
    imgClassName: 'absolute right-0 bottom-0 md:w-96 w-60',
    titleClassName: 'justify-center md:justify-start lg:justify-center ',
    img: '/b5.svg',
    spareImg: '/grid.svg',
  },
  {
    id: 6,
    title: 'Do you want to start a project together?',
    description: '',
    className: 'lg:col-span-2 md:col-span-3 md:row-span-1 md:h-[20vh] ',
    imgClassName: '',
    titleClassName:
      'justify-center md:max-w-full max-w-60 text-center text-4xl',
    img: '',
    spareImg: '',
  },
]

export const projects = [
  {
    id: 1,
    title: 'Jwellery Website ',
    des: 'Experience the sparkle of elegance with our stunning jewelry website, built using the MERN stack and styled beautifully with Tailwind CSS.',
    img: '/p1.png',
    iconLists: [
      '/re.svg',
      '/tail.svg',
      '/javascript.svg',
      '/node.svg',
      '/express.svg',
      '/mon.svg',
    ],
    link: 'https://jwellerey.onrender.com/',
  },
  {
    id: 2,
    title: 'Dabhand Solution',
    des: 'A REAL Software-as-a-Service app with AI features and a payments and credits system using the latest tech stack.',
    img: '/P2.PNG',
    iconLists: [
      '/re.svg',
      '/tail.svg',
      '/javascript.svg',
      '/node.svg',
      '/express.svg',
      '/mon.svg',
      '/razorpay.svg',
    ],
    link: 'https://dabhand.onrender.com/',
  },
  {
    id: 4,
    title: 'Chat Application',
    des: 'Experience seamless real-time communication with our MERN-based chat app, enhanced by Socket.IO and styled using Daisyui , Tailwind CSS.',
    img: '/p4.svg',
    iconLists: [
      '/re.svg',
      '/javascript.svg',
      '/node.svg',
      '/express.svg',
      '/tail.svg',
      '/daisy.svg',
      '/socket.svg',
      '/mon.svg',
    ],
    link: 'https://chatapp-6vdf.onrender.com/',
  },
  {
    id: 3,
    title: 'Ecommerece Clothing Store',
    des: 'Step into style with our MERN-based ecommerce clothing store, designed with Tailwind CSS for a sleek, responsive experience.',
    img: '/P3.PNG',
    iconLists: [
      '/re.svg',
      '/tail.svg',
      '/javascript.svg',
      '/node.svg',
      '/express.svg',
      '/mon.svg',
    ],
    link: 'https://mernstackproject-frontend-jlu9.onrender.com/',
  },
]

export const companies = [
  {
    id: 1,
    name: 'cloudinary',
    img: '/cloud.svg',
    nameImg: '/cloudName.svg',
  },
  {
    id: 2,
    name: 'appwrite',
    img: '/app.svg',
    nameImg: '/appName.svg',
  },
  {
    id: 3,
    name: 'HOSTINGER',
    img: '/host.svg',
    nameImg: '/hostName.svg',
  },
  {
    id: 4,
    name: 'stream',
    img: '/s.svg',
    nameImg: '/streamName.svg',
  },
  {
    id: 5,
    name: 'docker.',
    img: '/dock.svg',
    nameImg: '/dockerName.svg',
  },
]

export const workExperience = [
  {
    id: 1,
    title: 'Frontend Engineer Intern',
    desc: 'Assisted in the development of a web-based platform using React.js, enhancing interactivity.',
    className: 'md:col-span-2',
    thumbnail: '/exp1.svg',
  },
  {
    id: 2,
    title: 'Freelance Web Dev Project',
    desc: 'Led the dev of a web app for a client, from initial concept to deployment on app stores.',
    className: 'md:col-span-2',
    thumbnail: '/exp2.svg',
  },
  {
    id: 3,
    title: 'Mobile App Dev - JSM Tech',
    desc: 'Designed and developed mobile app for both iOS & Android platforms using React Native.',
    className: 'md:col-span-2',
    thumbnail: '/exp3.svg',
  },
  // {
  //   id: 4,
  //   title: 'Lead Frontend Developer',
  //   desc: 'Developed and maintained user-facing features using modern frontend technologies.',
  //   className: 'md:col-span-2',
  //   thumbnail: '/exp4.svg',
  // },
]
export const socialMedia = [
  {
    id: 1,
    img: '/git.svg',
    link: 'https://github.com/sumitkainturaji',
  },
  {
    id: 2,
    img: '/twit.svg',
    link: 'https://x.com/james_kim87',
  },
  {
    id: 3,
    img: '/link.svg',
    link: 'https://www.linkedin.com/in/sumit-kaintura-9682a7221?',
  },
  {
    id: 4,
    img: '/insta.svg',
    link: 'https://www.instagram.com/sumit__kaintura/',
  },
]
